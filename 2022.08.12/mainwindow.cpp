...
void MainWindow::on_pushButton_save_clicked()
{
    ...
    if(qry.exec())
    {
        ...
        QSqlQueryModel * modal=new QSqlQueryModel();
        QSqlQuery * qry_table=new QSqlQuery(mydb);
        qry_table->prepare("SELECT * from TB_PATIENT_INFO_2");
        qry_table->exec();
...
void MainWindow::on_pushButton_Edit_clicked()
{
    ...
    if(qry.exec())
    {
        ...
        //QSqlQueryModel * modal=new QSqlQueryModel(); //table 띄우도록
        //QSqlQuery * qry_table=new QSqlQuery(mydb);
        //qry_table->prepare("SELECT * from TB_PATIENT_INFO_2");
        //qry_table->exec();
        //ui->tableView->(modal); // -> 이 모델을 tabel view에 set; // table load를 위한 쿼리(신환후 바로 업데이트 된 table 띄우기 위함)
        connClose(); // 위 쿼리 성공적으로 실행됐으면 DB연결 닫기
    }
    else
    {

...
public:
    explicit FilterProxyModel(QObject* parent= nullptr):
        ...
        RegExp.setPatternOptions(Qt::CaseInsensitive);
        RegExp.setPattern(QRegularExpression::RegExp);
    }
    bool filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const  override{
        QModelIndex Index= sourceModel()->index(sourceRow, 0, sourceParent);

        QString qstr= sourceModel()->data(Index).toString();
        return (qstr.contains(RegExp));
    }
public slots:
    void setFilter(const QString& regExp){
        RegExp.setPattern(regExp);
        invalidateFilter();
    }
private:

        ...
        if(qry.exec()){ // 쿼리문에 오류가 없다면
            ...
            if(exist.next()){
                ...
                if(exist.value(0).toInt()){
                    QMessageBox::information(this,tr("오류"),tr("이미 존재하는 환자입니다. 신규환자 등록은 '신환' 버튼을 클릭한 후, 정보를 입력한 후에 저장을 버튼을 클릭하여 등록하시오.")); // 경고문 메시지 박스 다이어로그 띄우기
                }
                else{
...
void MainWindow::on_tableView_clicked(const QModelIndex &index)
{
    ...
    int iage = (icurrentdate - ibirthdate)/10000; // 만나이 계산 하여 iage에 저장

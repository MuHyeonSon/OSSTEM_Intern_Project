...
void MainWindow::on_lineEdit_search_returnPressed()
{
    //enter가 입력되면 lineEdit_search에 입력되어 있는 값을 가져옴
      //QString input_value;
      //input_value = ui->lineEdit_pntuid->text(); // lineEdit_search로 부터 가져온 값 input_value에 저장
    // input_value에 따라 filtering 하여 결과를 table에 출력
    //QSqlTableModel modal; // = new QSqlTableModel;
    connOpen();
    QSqlTableModel *modal = new QSqlTableModel;
    modal->setEditStrategy(QSqlTableModel::OnManualSubmit);
    modal->setTable("TB_PATIENT_INFO_2");
    //modal->insertRow("TB_PATIENT_INFO_2");
    //modal.select();
    //while (modal.canFetchMore()) //256row limit 문제해결방안(FetchMore)
    //   modal->fetchMore();
    //modal->setSourceModel(modal)
    ...
    ui->tableView->setModel(FilterModel); //tableView

...
void MainWindow::on_pushButton_save_clicked()
{
    ...
    if(!mydb.open()){ //DB 연결되어 있지 않다면 알림 출력
        ...
        int number_Exist = qry("SELECT COUNT(*) FROM TB_PATIENT_INFO_2");

        qDebug()<< "환자이미 존재(1) 존재x(0) (qry.prepare) : " << qry.prepare("SELECT COUNT(*) FROM TB_PATIENT_INFO_2 where pnt_uid = '"+pntuid+"'");
...
void MainWindow::on_pushButton_delete_clicked()
{
    if(!mydb.open()){ //DB 연결되어 있지 않다면 알림 출력
        ...
        qry.prepare("DELETE FROM TB_PATIENT_INFO_2 where pnt_uid='"+pntuid+"'"); // 입력으로 들어온 차트번호에 해당하는 환자정보 삭제시키는 쿼리문
...
// 환자 목록 클릭시 입력란에 해당 환자정보 띄우기 위한 멤버함수 정의 ( 쿼리문이 아니라 Q~mapped 사용 가능.)
...
void MainWindow::on_pushButton_Load_DB_clicked()
{
    ...
    {
        QMessageBox::information(this,tr("오류"),tr("선택하신 파일의 포맷은 db가 아닙니다. 파일 포맷을 다시 확인하고 시도하십시오."));
        //QString path = "C:/Users/Osstem/Documents/patient_management_program/PATIENT_INFO.db";
    }
    else{
